(function() {
  ...
  angular
        ...
        $('a[href="#upload-excel"]').on('click', function(e) {
          ...
          '<div class="well">' +
            '<h4 class="lighter">' +
              ...
              '<a href="song.php?action=import_example" data-toggle="modal" class="pink"> 下載匯入檔案 </a>' +
            '</h4>' +
    ...
    .controller('SongFormCtrl', function() {
      ...
      require(['mCustomScrollbar', 'div-option', 'serializeObject'], function() {
        ...
        $('#define-book-items').mCustomScrollbar({
          theme: 'light-thick',
        ...
        $('#search-book-items').mCustomScrollbar({
          theme: 'light-thick',
          scrollbarPosition: 'outside',
          autoExpandScrollbar: true,
          mouseWheel: {
        ...
        divOption.params.song_id = $('[name="song_id"]', $theForm).val();

        divOption.callback = function(res) {

          if (res.reload_define_book) {
            $.ajax({
              type: 'POST',
              dataType: 'json',
              url: divOption.url,
              data: $.extend(divOption.params, {
                action: 'load_define_book',
              }),
              cache: false,
              success: function(data, textStatus) {
                if (data.error == 0) {
          ...
          if (res.load_define_book) {
            //  .scroll-content
            $('#define-book-items .mCSB_container').html(res.content);
            return;
          }

          if (res.find_define_book) {
            $('#search-book-items .mCSB_container').html(res.content);
            return;
          }
        };
        divOption.callback({reload_define_book: true});

        /* 搜尋歌曲 */
        $('#search-book-button').on('click', function(e) {
          $.ajax({
            type: 'POST',
            dataType: 'json',
            url: divOption.url,
            data: $.extend(divOption.params, {
              action: 'search_book',
            }, $('.ajax-book-search').serializeObject()),
            cache: false,
            success: function(data, textStatus) {
              if (data.error == 0) {
                divOption.callback(data);
        ...
        $('#join-checked-true-button').on('click', function(e) {
          e.preventDefault();
          $('input[name^="join_book"]').prop('checked', true);
        });
        $('#join-checked-false-button').on('click', function(e) {
          e.preventDefault();
          $('input[name^="join_book"]').prop('checked', false);
        });
        $('#join-define-book').bind('click', function() {
          var errArr = [];

          if ($('input[name^="join_book"]:checked').size() == 0) {
          ...
          if (errArr.length > 0) {
            ...
            $.ajax({
              ...
              url: divOption.url,
              data: $.extend(divOption.params, {
                action: 'add_define_book',
              }, $('.ajax-add-define-book').serializeObject()),
              cache: false,
              success: function(data, textStatus) {
                if (data.error == 0) {
                  divOption.callback(data);
        ...
        $('#update-checked-true-button').on('click', function(e) {
          e.preventDefault();
          $('input[name^="update_book"]').prop('checked', true);
        });
        $('#update-checked-false-button').on('click', function(e) {
          e.preventDefault();
          $('input[name^="update_book"]').prop('checked', false);
        });

        $('#drop-define-book').on('click', function(e) {
          if ($('input[name^="update_book"]:checked').size() == 0) {
            ...
            $.ajax({
              ...
              url: divOption.url,
              data: $.extend(divOption.params, {
                action: 'drop_define_book',
              }, $('.ajax-update-define-book').serializeObject()),
              cache: false,
              success: function(data, textStatus) {
                if (data.error == 0) {
                  divOption.callback(data);
        ...
        $('#define-book-items').on('click mouseover mouseout', '[data-cell-edit]', function(e) {
          var $editObj = $(this);
          if (e.type == 'mouseover') {
            ...
            var _bookId = $editObj.closest('tr').data('item-id');
